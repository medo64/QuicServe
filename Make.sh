...
function dist() {
    ...
    for DIRECTORY in "Makefile" "Make.sh" "CONTRIBUTING.md" "ICON.png" "LICENSE.md" "README.md" "src"; do
...
APP_NAME=`cat "$BASE_DIRECTORY/src/QuicServe.csproj" | grep "<AssemblyName>" | sed 's^</\?AssemblyName>^^g' | xargs`
APP_VERSION=`cat "$BASE_DIRECTORY/src/QuicServe.csproj" | grep "<Version>" | sed 's^</\?Version>^^g' | xargs`
